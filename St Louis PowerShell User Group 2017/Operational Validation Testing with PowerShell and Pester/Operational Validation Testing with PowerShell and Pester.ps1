...
<#
    The code in this region was stolen, I mean borrowed from Thomas Rayner (@MrThomasRayner). For more information,
    see: http://mikefrobbins.com/2017/11/02/safety-to-prevent-entire-script-from-running-in-the-powershell-ise/
#>
...
<#
    ...
    Presentation from St Louis PowerShell User Group Meetup 2017
...
#Import the MrToolkit module (Available in the PowerShell Gallery: Install-Module -Name MrToolkit)
Import-Module U:\GitHub\PowerShell\MrToolkit\MrToolkit.psd1

#VM's used: DC01, SQL14, SQL16, Win10-1703 (A domain controller, two SQL Servers, and a Windows 10 pc.)
...
#It's an opensource PowerShell module that ships with Windows 10 (Pester version 3.4.0).
...
Start-Process http://mikefrobbins.com/2016/07/14/powershell-function-for-creating-a-powershell-function-template/
...
Invoke-Pester -Show None -PassThru
...
#Run the Pester test
Invoke-Pester -Show None -PassThru | Select-Object -Property TotalCount, PassedCount, FailedCount, SkippedCount
...
Invoke-Command -ComputerName SQL16 {Stop-Service -Name SQLServerAgent -Force}

#Run the Pester test
Invoke-Pester

#Stop the SQL Server service
Invoke-Command -ComputerName SQL16 {Start-Service -Name SQLServerAgent}
