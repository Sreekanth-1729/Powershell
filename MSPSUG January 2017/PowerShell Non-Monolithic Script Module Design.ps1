...
Presentation for the Mississippi PowerShell User Group - January 10th, 2017
...
#Check to see if the function exist on the Function PSDrive
...
#Determine what commands are exported
...
Get-ChildItem -Path $env:ProgramFiles\WindowsPowerShell\Modules\MyModule\*.* | ForEach-Object {psEdit -filenames $_}
...
Get-ChildItem -Path $env:ProgramFiles\WindowsPowerShell\Modules\MyModule\*.ps1 | ForEach-Object {psEdit -filenames $_}
...
Get-MrFunctionsToExport -Path $env:ProgramFiles\WindowsPowerShell\Modules\MyModule | clip
