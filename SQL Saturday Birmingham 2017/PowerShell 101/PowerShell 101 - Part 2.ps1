...
#-- Begin Remove
#5 VM's are used during this demonstration. 2 running Windows 8.1,
...
#Move to PC01 VM
#PC01 has the SQLPS module and the RSAT tools installed
...
Invoke-Command -ComputerName DC01, SQL02, SQL04 {
...
Invoke-Sqlcmd -ServerInstance sql04 -Database master -Query '
select name, database_id, compatibility_level, recovery_model_desc from sys.databases'

#Filtering

Invoke-Sqlcmd -ServerInstance SQL04 -Database AdventureWorks2012 -Query '
...
Invoke-Sqlcmd -ServerInstance sql04 -Database master -Query 'EXEC sp_databases'

#This will generate an error because there are 2 columns with the name SPID
Invoke-Sqlcmd -ServerInstance sql04 -Database master -Query 'EXEC sp_who2'

#endregion

#region Working with SQL Server thorugh the use of SMO (SQL Management Objects)

$SQL = New-Object –TypeName Microsoft.SqlServer.Management.Smo.Server -ArgumentList 'sql04'
...
Get-PSProvider
Get-PSDrive
Get-ChildItem -Path SQLServer:\SQL\SQL04\Default\Databases
Get-ChildItem Env:
dir Variable:
...
Get-SqlDatabase -ServerInstance SQL04

#endregion

#region .NET Framework

Invoke-MrSqlDataReader -ServerInstance sql04 -Database msdb -Query "
...
#Query the AdventureWorks2012 database on SQL04
Invoke-Sqlcmd -ServerInstance sql04 -Database AdventureWorks2012 -Query '
...
Measure-Command {Invoke-Sqlcmd -ServerInstance sql04 -Database AdventureWorks2012 -Query '
select Employee.LoginID,
       Person.FirstName as givenname,
       Person.LastName as surname,
       Employee.JobTitle as title,
