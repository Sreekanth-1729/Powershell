...
#Move to Win10-1607 VM
#Win10-1607 has the SQLPS module and the RSAT tools installed
...
Invoke-Command -ComputerName DC01, SQL02, SQL2016 {
...
Invoke-Sqlcmd -ServerInstance sql2016 -Database master -Query '
select name, database_id, compatibility_level, recovery_model_desc from sys.databases'

#Filtering

Invoke-Sqlcmd -ServerInstance SQL2016 -Database AdventureWorks2014 -Query '
...
Invoke-Sqlcmd -ServerInstance sql2016 -Database master -Query 'EXEC sp_databases'

#This will generate an error because there are 2 columns with the name SPID
Invoke-Sqlcmd -ServerInstance sql2016 -Database master -Query 'EXEC sp_who2'

#endregion

#region Working with SQL Server thorugh the use of SMO (SQL Management Objects)

$SQL = New-Object –TypeName Microsoft.SqlServer.Management.Smo.Server -ArgumentList 'sql2016'
...
Get-PSProvider
Get-PSDrive
Get-ChildItem -Path SQLServer:\SQL\SQL2016\Default\Databases
Get-ChildItem Env:
dir Variable:
...
Get-SqlDatabase -ServerInstance SQL2016

#endregion

#region .NET Framework

Invoke-MrSqlDataReader -ServerInstance sql2016 -Database msdb -Query "
...
Measure-Command {Invoke-Sqlcmd -ServerInstance sql2016 -Database AdventureWorks2014 -Query '
select Employee.LoginID,
       Person.FirstName as givenname,
       Person.LastName as surname,
       Employee.JobTitle as title,
